...
| `VITE_PUBLIC_SITE_ORIGIN` | Origin of the `memoza-sites` public-page host: builds a published page's shareable link (`<origin>/<username>/<page_no>`) and the `format:html` sandbox runner URL (`<origin>/_runner`); defaults to `https://memozasites.com` if unset |
